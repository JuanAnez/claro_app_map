...
import 'package:icc_claro_app/features/features/map/data/database/tables/icc_antennas_table.dart';
...
class AntennasDAO {
  ...
  return data.map((row) => IccAntennasTable.fromMapToEntity(row)).toList();
}
  ...
  Future<int> insertAntenna(AntennaEntity antenna) async {
    ...
    return await db.rawInsert("""
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      """, antenna.toWhereArgs()); // Método corregido
  }
